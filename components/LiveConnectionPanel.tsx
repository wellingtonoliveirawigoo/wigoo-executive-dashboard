...
const LiveConnectionPanel: React.FC<Props> = ({ onDataLoaded, isLoading, setIsLoading, viewMode, lockedClient }) => {
  ...
  const handleSync = async () => {
    ...
    setIsLoading(true);
    try {
      const exportString = await executePowerBiQuery(client, startDate, endDate, viewMode);
      if (!exportString) {
        throw new Error('Nenhum dado retornado do Power BI');
      }
      ...
      console.error(error);
      alert(error instanceof Error ? error.message : 'Erro na conexão com o Power BI');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-end gap-6">
        <div className="flex-grow space-y-4 w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-black text-gray-900 dark:text-white leading-tight">Live Power BI Connect</h3>
              <p className="text-[10px] text-gray-400 dark:text-wigoo-light/30 uppercase tracking-widest mt-0.5">Sincronização em Tempo Real</p>
            </div>
          </div>
